...
def logistic_regression(X, y, reg=1.0, method='SGD', lr=1e-3, tol=1e-3, max_iter=1000):
    if method == 'SGD':
        return SGD(X, y, logistic_SGD_grad, logistic_loss, lr, tol, max_iter)
    elif method == 'GD':
        return GD(X, y, logistic_GD_grad, logistic_loss, lr, tol, max_iter)
    else:
...
def evaluate(filename, x_cols, y_col, reg, method, transform=None, lr=1e-2, tol=1e-3, max_iter=1000):
    ...
    weights = logistic_regression(X_train, y_train, reg, method, lr, tol, max_iter)
    ...
    print(model_str(x_cols, y_col, True, 'logistic')+f" using {method} logistic regression with {transform} normalization")
    ...
    for method in ["SGD", "GD"]:
        for x_cols in features:
            for normalization in ["standard", "min_max"]:
                evaluate("../Data/binary_classification.csv", x_cols, y_col, 10, method, normalization)